...
-- ─── HYPERLINKS ───────────────────────────────────────────────────
-- Enable clickable links (cmd+click to open in Microsoft Edge)
config.hyperlink_rules = wezterm.default_hyperlink_rules()

-- Add additional patterns for URLs without http/https prefix
table.insert(config.hyperlink_rules, {
	regex = [[\bwww\.\S+\.\S+\b]],
	format = "https://$0",
})

-- Force all links to open in Microsoft Edge
wezterm.on("open-uri", function(window, pane, uri)
	wezterm.run_child_process({ "open", "-a", "Microsoft Edge", uri })
